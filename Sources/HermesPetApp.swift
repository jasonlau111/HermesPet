...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func updateAll() {
        ...
        if let button = statusItem?.button {
            ...
            case .connected:
                button.image = Self.statusBarIcon(accessibilityDescription: "Hermes — 已连接")
    ...
    private static func statusBarIcon(accessibilityDescription: String) -> NSImage {
        ...
        outer.move(to: NSPoint(x: 4.1, y: 4.3))
        outer.curve(to: NSPoint(x: 3.4, y: 10.5),
                    controlPoint1: NSPoint(x: 2.8, y: 6.1),
                    controlPoint2: NSPoint(x: 2.7, y: 8.8))
        outer.curve(to: NSPoint(x: 8.6, y: 15.1),
                    controlPoint1: NSPoint(x: 4.3, y: 13.3),
                    controlPoint2: NSPoint(x: 6.2, y: 15.1))
        outer.curve(to: NSPoint(x: 14.7, y: 9.1),
                    controlPoint1: NSPoint(x: 12.0, y: 15.3),
                    controlPoint2: NSPoint(x: 15.7, y: 12.5))
        outer.curve(to: NSPoint(x: 13.2, y: 4.2),
                    controlPoint1: NSPoint(x: 15.2, y: 6.7),
                    controlPoint2: NSPoint(x: 14.7, y: 5.0))
        outer.stroke()

        let face = NSBezierPath(ovalIn: NSRect(x: 5.0, y: 3.8, width: 8.2, height: 8.3))
        face.lineWidth = 1.4
        face.stroke()

        let bangs = NSBezierPath()
        bangs.lineWidth = 1.4
        bangs.lineCapStyle = .round
        bangs.lineJoinStyle = .round
        bangs.move(to: NSPoint(x: 4.7, y: 10.6))
        bangs.curve(to: NSPoint(x: 8.1, y: 10.2),
                    controlPoint1: NSPoint(x: 5.8, y: 11.4),
                    controlPoint2: NSPoint(x: 7.0, y: 11.3))
        bangs.curve(to: NSPoint(x: 12.0, y: 10.8),
                    controlPoint1: NSPoint(x: 9.2, y: 11.3),
                    controlPoint2: NSPoint(x: 10.5, y: 11.6))
        bangs.stroke()

        NSBezierPath(ovalIn: NSRect(x: 6.9, y: 7.4, width: 1.0, height: 1.2)).fill()
        NSBezierPath(ovalIn: NSRect(x: 10.1, y: 7.4, width: 1.0, height: 1.2)).fill()

        let mouth = NSBezierPath()
        mouth.lineWidth = 0.9
        mouth.lineCapStyle = .round
        mouth.move(to: NSPoint(x: 8.6, y: 5.9))
        mouth.curve(to: NSPoint(x: 9.5, y: 5.9),
                    controlPoint1: NSPoint(x: 8.9, y: 5.5),
                    controlPoint2: NSPoint(x: 9.2, y: 5.5))
        mouth.stroke()
